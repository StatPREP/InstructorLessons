...
# USER_CODE <- "date ~ brths" # for testing
code <- for_checkr(USER_CODE)
line_where(code, 
           insist(same_name(F, "~"), 
                   "The tilde (~) is missing. Formulas always involve a tilde."),
           failif(V == (date ~ births), 
                  "You need to have the y-axis variable on the left and the x-axis variable on the right"),
           insist(all(sort(all.vars(V)) %in% c("births", "date")),
                      "You want the variables `births` and `date` in the formula. You've got {{paste0(\"`\", all.vars(V), \"`\", collapse = \", \")}}."),
           insist("births" %in% all.vars(f_lhs(V)), "The `births` variable needs to be on the left side of the tilde, not `{{all.vars(f_lhs(V))}}`."), 
           insist("date" %in% all.vars(f_rhs(V)), "The `date` variable needs to be on the right side of the tilde, not `{{all.vars(f_rhs(V))}}`."), 
           passif(V == (births ~ date), "Good. That's all this exercise was about: typing a formula." ))
```
...
qstats(births ~ wday, data = Births1978, mean)
```
