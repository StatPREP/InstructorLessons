...
* **`function`** will be replaced with the name of the operation you want to perform. There are a couple dozen  such operations that you will encounter frequently. Here, consider these two:    
    1. `qstats` which calculates basic statistics.
...
* **`...`**. Many functions take additional arguments specifying details of the operation. What these details are depends on the operation, and not all functions require additional arguments. For instance, the `qstats()` function you will use next takes an additional argument specifying what statistic (e.g. the mean) to calculate.
...
qstats( ~ height, data = Galton,  mean)
```
...
qstats(height ~ sex, data = Galton, mean)
```
